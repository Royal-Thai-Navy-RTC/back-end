...
CREATE TABLE `TeachingSchedule` (
    ...
    `battalionCode` VARCHAR(32) NULL,
    `color` VARCHAR(16) NOT NULL DEFAULT '#1E90FF',
    ...
    INDEX `TeachingSchedule_battalionCode_companyCode_start_idx`(`battalionCode`, `companyCode`, `start`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `TeacherLeave` (
    ...
    `status` ENUM('PENDING', 'APPROVED', 'REJECTED', 'CANCEL') NOT NULL DEFAULT 'PENDING',
    `isOfficialDuty` BOOLEAN NOT NULL DEFAULT false,
    `adminApprovalStatus` ENUM('PENDING', 'APPROVED', 'REJECTED', 'CANCEL') NULL,
    `adminApprovalBy` INTEGER NULL,
    `adminApprovalAt` DATETIME(3) NULL,
    `ownerApprovalStatus` ENUM('PENDING', 'APPROVED', 'REJECTED', 'CANCEL') NULL,
...
CREATE TABLE `LibraryItem` (
    ...
    INDEX `LibraryItem_isActive_idx`(`isActive`),
    INDEX `LibraryItem_updatedAt_idx`(`updatedAt`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
