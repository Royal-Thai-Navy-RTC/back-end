...
- `GET /admin/soldier-intakes` (ADMIN) — query `page,pageSize,search,battalionCode,companyCode,specialSkillFilter=(has|no),healthFilter=(has|no),religionFilter=(พุทธ|อิสลาม|คริสต์|อื่นๆ),provinceFilter,educationFilter,bloodFilter,serviceYears` (ค้นหา firstName,lastName,citizenId,phone) — `อื่นๆ` จะแสดงศาสนาที่ไม่ใช่ พุทธ/อิสลาม/คริสต์
- `GET /admin/soldier-intakes/export` (ADMIN) — ส่งออก Excel รายการทหารใหม่; รองรับ `search,battalionCode,companyCode,specialSkillFilter=(has|no),healthFilter=(has|no),religionFilter=(พุทธ|อิสลาม|คริสต์|อื่นๆ),provinceFilter,educationFilter,bloodFilter,serviceYears` เหมือนหน้า list (เฉพาะ `อื่นๆ` จะไม่รวม พุทธ/อิสลาม/คริสต์)
