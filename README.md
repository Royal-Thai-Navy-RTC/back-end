...
```
- `GET /teaching-schedules` (เปิด public) — query `start,end,teacherId,page,pageSize<=200`; คืน `{ data, page, pageSize, total, totalPages }` เรียงตามเวลา (กรองช่วงวันที่เพื่อโหลดเฉพาะที่ต้องแสดง เช่น view ของ FullCalendar)  
